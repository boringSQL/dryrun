...
The peer section is captured on both primaries and standbys, but the lag reference LSN branches by role: `pg_current_wal_lsn()` on a primary, `pg_last_wal_replay_lsn()` on a standby, so cascading replicas are visible without aborting the capture. Lag fields are NULL for an idle standby that has not sent a recent replay message, so a nil lag means "unknown", not "no lag". The `pg_read_all_stats` role is required for full peer visibility; without it `pg_stat_replication` returns zero rows without error, so `replication_peers_read_ok=true` with an empty list is ambiguous between "no standbys" and "no privilege". To join peer rows to `describe_table` / `detect` node breakdowns, set each replica's `--label` to its `application_name` (the `primary_conninfo` setting on the replica). Peer rows also carry `client_addr` (replica IP or NULL for Unix sockets); that is infrastructure metadata, not table data, and is not routed through the column-oriented data-masking policy.
