...
func (s *Server) registerSchemaTools(srv *mcpserver.MCPServer) {
	...
	srv.AddTool(
		mcp.NewTool("check_migration",
			mcp.WithDescription("Before any ALTER TABLE, CREATE INDEX, DROP, or other DDL reaches a database: the lock level each statement takes, whether it rewrites the table, and rollback_ddl where one applies. Tables created earlier in the same input are recognized as empty, so statements against them are not flagged for scans or rewrites they cannot cause. Where the safe form is mechanical it returns safer_sql -- the rewrite as statements to run in order, each on its own rather than wrapped in one transaction. When at least one statement is unsafe and every unsafe statement has one, migration_sql bundles the whole input as one runnable file -- unsafe statements replaced by safer_sql, safe ones passed through unchanged; it is absent otherwise, since a file that still holds an unsafe statement (including a statement type this tool cannot analyze) would be worse than no file at all. ddl may be a pasted migration file: goose (-- +goose Up/Down), dbmate (-- migrate:up/down) and tern (---- create above / drop below ----) markers are recognized, and direction (default up) picks the half to analyze; migration_sql is re-wrapped to stay runnable by that runner. Needs no database connection and applies nothing. It does not estimate wall-clock duration, and not every unsafe statement has a rewrite: a column type change or a backfill needs a batch size and a deploy order this tool cannot decide for you."),
			mcp.WithString("ddl", mcp.Required(), mcp.Description("DDL statement, or a whole goose/dbmate/tern migration file.")),
			mcp.WithString("direction",
				mcp.Enum("up", "down"),
				mcp.DefaultString("up"),
				mcp.Description("Which half of a migration file to check: 'up' (default) or 'down'. Plain SQL has no down section, so direction='down' errors there."),
			),
			mcp.WithRawOutputSchema(checkMigrationOutputSchema),
			annSnapshot,
		),
...
func (s *Server) registerHistoryTools(srv *mcpserver.MCPServer) {
	srv.AddTool(
		mcp.NewTool("snapshot_diff",
			mcp.WithDescription("When something changed and you need to know what: the schema DDL delta between two snapshots, plus planner sizing, activity, and query-shape drift correlated to the same capture window. Query deltas are per shape over that window, so their mean is comparable across captures -- unlike pg_stat_statements' own since-reset mean. Needs no database connection, but does need two snapshots in the local history. Snapshot-to-snapshot only, never against the live database (check_drift); it reports what moved, not why. Totals are a delta over the interval between the two anchors, not a rate: capture cadence is irregular, so deltas over unequal intervals do not compare. Each per-node query delta reports that interval as `window` in nanoseconds (window_minutes is a different number: how far a planner or activity capture may sit from an anchor to be correlated to it); planner and activity deltas carry no interval, so use the anchors' taken_at for those. If you divide by it: only when incomparable is empty and window is above zero, and never for an entry whose status is reset or truncated -- their delta is a cumulative counter rather than the window's work, which is why this tool leaves them out of its own totals and counts them in not_subtractable. Read caveats. An activity or query delta with incomparable set was refused (one label covered two servers) and carries no rows; it is not a quiet node. _meta.history, when present, says what the local store holds per stream and over what span."),
	...
	srv.AddTool(
		mcp.NewTool("list_top_queries",
			mcp.WithDescription("When asked what this database spends its time on: captured pg_stat_statements query shapes ranked by exec time, calls, or mean. Needs no database connection; reads the local history (dryrun snapshot capture --streams query, or init's best-effort capture) and is empty when no capture carries query stats. Each entry is tagged with its reporting node and never averaged across nodes: a primary and a replica are different workloads. A node label that rotates between servers reports only the server that answered its newest capture; _meta.hint says so when the captures show it. SQL is qshape-normalized and parameterized, never the literal text or parameter values a user ran. Counters are cumulative since the last reset, not a recent-activity rate. It reports the newest capture per node and differences nothing, so it says what the corpus has cost since that reset, never what it cost this week; snapshot_diff kind=query is what differences them, and _meta.history, when present, says what the local store holds. An empty result carrying unknown: true means no capture holds query stats at all, which is not the same as a database whose statements are all cheap. Before comparing or differencing any two numbers, read _meta.hint -- it carries the caveats that decide whether they are comparable at all."),
