...
var (
	...
	snapshotDiffOutputSchema = json.RawMessage(`{
		"type": "object",
		"description": "Diff between two snapshots: refs, headline summary, ranked per-object deltas for the requested kind, correlated planner/activity drift, and per-node query-shape drift (query_delta) whose means are over the window rather than since pg_stat_statements last reset; entries carry buffer counters and blocks-per-call means, with block_size once per node delta. Full view adds raw per-row deltas.",
		"properties": {
			` + metaProperty + `
		},
		"additionalProperties": true
